...
{
    /* NOTE(mehmet): This is not really a window handler per se, it's actually just
...
{
    /* NOTE(mehmet): This is an example of an actual window procedure. It doesn't do anything
    ...
    */
    ...
    switch (Message)
    {
        // NOTE(mehmet): Mildly annoying, if you want to specify a window, you have
        ...
        // NOTE(mehmet): Anything you want the application to handle, forward to the main thread
        // here.
        case WM_MOUSEMOVE:
        case WM_LBUTTONDOWN:
        case WM_LBUTTONUP:
...
{
    /* NOTE(Mehmet): This is your app code. Basically you just do everything the same,
...
{
    /* NOTE(mehmet): At startup, you create one hidden window used to handle requests
    ...
    */
    ...
    // NOTE(mehmet): Once the service window is created, you can start the main thread,
    // which is where all your app code would actually happen.
    CreateThread(0, 0, MainThread, ServiceWindow, 0, &MainThreadID);

    // NOTE(mehmet): This thread can just idle for the rest of the run, forwarding
